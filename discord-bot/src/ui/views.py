...
class LobiView(View):
    ...
    async def profil_bak(self, interaction: discord.Interaction, button: Button):
        from src.economy.db import ensure_oyuncu, get_oyuncu, get_liderlik
        ...
        lider = await get_liderlik("cip", 5)
        siralam = next((i+1 for i, o in enumerate(lider) if o["user_id"] == interaction.user.id), "10+")
        embed = discord.Embed(title=f"📊 {interaction.user.display_name} — Profil", color=0x2ecc71)
        embed.add_field(name="🏅 Sıralama", value=f"#{siralam}", inline=True)
...
class TasAtModal(Modal, title="🗑️ Taş At"):
    def __init__(self, masa_id: str):
        super().__init__()
        self.masa_id = masa_id

    tas_no = TextInput(
        label="Atmak istediğiniz taşın numarası",
        placeholder="Örn: 5",
        min_length=1,
        max_length=2,
        required=True
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            index = int(self.tas_no.value.strip()) - 1
        except ValueError:
            await interaction.response.send_message("❌ Geçersiz numara.", ephemeral=True)
            return
        from src.game.manager import game_manager
        await game_manager.tas_at(interaction, self.masa_id, index)
